...
| `vertical-slice/` | `AI-internal/vertical-slice/` against the development file | re-run the runner | yes, 16 s — the model is deterministic |
...
- `batch-reports/` — one report per executed batch. Batches 1 (orientation and set-up),
  2 (Chap reconnaissance), 3 (the data), 4 (methods), 5 (the bootstrap) and 6 (the
  vertical slice).
...
- `vertical-slice/` — what the first model of our own scored, end to end, on the real
  development file at the fixed scheme, captured in batch 6. Fully regenerable in 16
  seconds, because unlike the reference this model is deterministic and its environment
  is locked. **None of its numbers is a reported result** — the claim tree is erected in
  batch 7, and the same model is re-run from a node there.
