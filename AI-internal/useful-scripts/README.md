...
| `cleanroom_holdout_reproduction.py` | What batch 27's clean-room run established about the **phase-E half**, which no earlier run reached: per model and per combination on both halves, the holdout distribution's reported fields archived beside the clean-room's, whether the environment was the lockfile's once ANSI colour is stripped from `uv pip freeze`, and the rows `pair_holdout_development.py` stopped the run on — separating a drifted *number* from a moved *pairing*, which its message conflates. Reads the preserved artefacts, so it still answers after the clone is gone. Writes to `AI-generated/validation/` | `/validate cleanroom` |
