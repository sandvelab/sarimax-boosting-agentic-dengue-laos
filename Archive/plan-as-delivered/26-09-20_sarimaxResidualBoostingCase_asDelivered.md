...
|---|---|---|---|
| 1 | A — Orient & set up | Read `readme-at-start.md`, `AGENTS.md`, `MOTIVATION.md`; re-verify the archived Laos dataset's checksums and re-read its `provenance.md`; stand up the repository's own `.venv`; scope the native Python environment (SARIMAX + CRPS + candidate stage-2 libraries) and start `/pin-environment`; write the root `analysis/claim.md`; raise any open questions this plan leaves the human (backtest scheme, whether to keep the EWARS citation, anything else). No modelling. | open |
...
| 12 | E — Final validation | Open the 2010 holdout once; run exactly the frozen manifest; report the held-out distribution beside the development one. | open |
| 13 | F — Claims & report | Build the claim collection from the tree; generate the hierarchical report. | open |
| 14 | F (cont.) | `/validate cleanroom` and `/validate outsider`; fix what they find. | open |
| 15 | F — Release | Write the manuscript section(s) this project supports; run the release scan; push. | open |
