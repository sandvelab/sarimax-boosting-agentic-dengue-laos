...
| 20 | F — Release | Write the manuscript section(s) this project supports; run the release scan; push. | blocked: everything but the push is done — the full manuscript, the reproducibility report, the release scan, the clean-room and outsider checks re-run at release, the hierarchical report rebuilt, the release commit tagged. **The push and the citable deposit await the human**, with the scan's findings (§4b, 2026-09-23) in front of them; `/release` never pushes without asking. |
