...
| | Decision |
|---|---|
| **Project seed** | `20260920`. Every component seed derives from it, by the same BLAKE2b construction the prior project used (implemented in `analysis/scripts/lib/project_seed.py`; the pointer to `provenance-record.md` that stood here until 2026-09-23 named a file that does not contain it — corrected at the outsider test's finding, §4b). |
...
### 2026-09-23 — batch 20: which instructions actually governed batches 1–19

| Decision | Basis | Agency |
|---|---|---|
| **Batches 1–19 ran with `/Users/geirksa_1_2_3/ai/CLAUDE.md` in context**, in addition to this repository's own `AGENTS.md`. That file is the parent vault-collection's cross-vault housekeeping instruction set (sync generalizable insights between sibling vaults' `CLAUDE.md` files, check vault folder structure, update `skills.md`). It contains nothing about modelling, thresholds, seeds or evaluation, and nothing in it was acted on; no result or number can have moved because of it. Recorded because `AGENTS.md` §3, Rule 4 makes the instruction files part of the method, and the published instruction set would otherwise be short by one file that lives outside the repository and does not ship with it. | Raised by `/validate outsider` in batch 19 and left open for the human; settled here at the human's instruction ("start using the AGENTS.md in this folder, not something outside the folder"). | human-set (that it is settled now); agent-retrieved (the observation, batch 19) |
| **The fix is `.claude/settings.local.json`, not the tracked `settings.json`.** The tracked file keeps its `<PARENT_DIR>` / `<HOME>` placeholders; the real absolute paths go in the gitignored local file. `setup-guide.md` §2 is corrected to say so. | `.claude/settings.json` is tracked and was pushed public at batch 1 (`1f0a147`). Substituting real paths there would publish the author's home directory to a public repository *and* still be wrong on every other machine, since absolute paths do not transfer — so a cloner would be no better protected than before. The placeholders are the template's contract with the next person who copies it (`setup-guide.md` §1: this repository began as a copy of `paper_starting_point`), and step 2 of that setup was simply never performed. | agent-autonomous |
| **`setup-guide.md` §2 also gains the failure mode that produced this**: `<PARENT_DIR>` taken literally is not always the right path. This project sits two levels below the directory holding the offending `CLAUDE.md` — its immediate parent (`special-purpose vaults/`) has none, and the file that loads is the *grandparent* (`ai/CLAUDE.md`). A literal substitution of the immediate parent leaves the exclusion matching nothing. | The same class as batch 16's `freeze` check and batch 19's `cleanroom.sh`: a safeguard that passes by matching nothing, and so reads as evidence while protecting nothing. Third instance recorded in this project. The sibling vault `politikk` carries the substituted-but-still-wrong version of exactly this, which is how the pattern was confirmed rather than assumed. | agent-autonomous |
| **Six sibling vaults carry the same unsubstituted template and are not touched from here** (`AgenticImmuneAnalytics`, `ReprodicbleAgenticAiCase`, `agentic-model-development-start`, `oldReprodicbleAgenticAiCase`, `run-ghr-model-in-chap`, and `politikk`'s near-miss) | `AGENTS.md` §1 and §9: this repository carries one project and does not grow work outside it. Recorded as a visible absence, for a session at the vault-collection root. | agent-autonomous |

### 2026-09-23 — batch 20: the release

| Decision | Basis | Agency |
|---|---|---|
| **The full manuscript is written from all 32 claims** (`Human-AI-collaboration/manuscript/26-09-23_twoStageDengueLaosManuscript.md`, with its sidecar), as a venue-neutral research article of about 7,100 words, and **it carries a section on the method of work** (§3.9, with a paragraph in the discussion and an agency statement as §8). | The two questions T16 left for the human — whether the article carries such a section, and whether the undecided venue shapes its length — were answered by default rather than left blocking: plan §1 makes the veridical record a deliverable of equal standing with the model, so a manuscript that omitted how the work was done would report half the project; and no venue is decided, so no venue's constraints were applied. Both are reversible by a `_v2` iteration. | agent-autonomous |
| **The eight references were re-verified by web search, and one characterisation corrected before the draft was committed.** The batch-10 literature record summarised Sesay et al. (2026) as a negative-binomial autoregression reaching near-nominal coverage "where a GLM did not"; the article's abstract says the near-nominal calibration belongs to a negative-binomial integer autoregression (INGARCH) against a negative-binomial GLM that substantially under-covered, and that lag-1 climate added no significant skill. The manuscript says the latter. The other seven were not re-read; the manuscript characterises them as the batch-10 record does, and its provenance section says so. | `AGENTS.md` §7: a named source is read before it is written about. Re-reading all eight in a release batch was judged not worth its cost against a record that had read them once; re-verifying the bibliographic details was cheap and found one correction. | agent-autonomous; information: agent-retrieved |
| **The release scanner exists (`release_scan.sh`) and its one automatic verdict is "failed"**, on a credential-shaped filename: `.claude/settings.local.json` was tracked from `1f0a147` (the prior project's first commit, 2026-08-23) until `15b4ba9`. Its content at that commit was three permission grants, one of them naming an absolute path under the author's home directory; no secret. **The history is not rewritten**, and the verdict stands in the record as what it is: a filename that matched, examined, and found to hold no credential. | Rewriting history would change every commit hash that every provenance record, freeze record and opening record cites, which is the addressing scheme Rule 4 exists for; and the content has been on the public remote since 2026-08-23. The scan is right to fail on shape and a reader is right to check content; both are recorded. Whether the residual matters is the human's call at the push. | agent-autonomous (the scanner and the assessment); the push: human |
| **The author's home-directory path appears in four tracked files and is reported, not redacted**: the batch-19 clean-room console log (a validation artefact, never hand-edited), the task-detail entry T16, the §4b entry of 2026-09-23 above, and `readme-at-start.md`'s settings table — all recording the same fact, which file was in context during batches 1–19. | The username in that path is the author name on all 266 commits since the first push, and the e-mail is in the same metadata, so the path discloses nothing the history does not. Redacting it would edit an append-only record and a generated artefact to remove information that is elsewhere anyway. Left to the human, who may prefer redaction in the two prose files. | agent-autonomous (report, not redact); human (whether to redact) |
| **Data permission is settled by checking the upstream terms rather than by the plan's assertion.** The source repository `dhis2/climate-health-data` carries no licence file; its README describes data published for direct download; the schema attributes the case counts to OpenDengue, whose site states CC BY 4.0. Attribution is appended to `Archive/lao-dataset/provenance.md` and `Archive/sibling-datasets/provenance.md`. The ERA5-Land, OCHA COD-AB and WorldPop terms for the covariate fields were not checked; those fields are on no main-path model. | `/release`'s second question is whether every included data file may lawfully be published, and "public and redistributable (plan §4)" was a decision, not a check. CC BY 4.0 permits redistribution with attribution, which the archive now carries. | agent-autonomous; information: agent-retrieved |
| **The pre-release grep for prior-project prose was run** (`chapenv`, `06_external`, `data-acquisition`, `$COMBO`, `05_stability`, `batch [23][0-9]`, `install-chap`, reconnaissance folders). Fixed: `.gitignore`'s entries for folders this project never had and its wrong installer name; the root `analysis/claim.md`'s "not yet pinned" environment line. Recorded and left: comments in `check_invariants.py` citing prior-project batch numbers and a dead `MANIFEST_EXTERNAL` path, and a dead headline block in `build_hierarchical_report.py` guarded by a file-existence check. `$COMBO` in `06_stability`'s provenance records is this project's own placeholder convention and is correct. | The lesson batch 19 recorded for this row. Changing the invariant checker or the report generator in a release batch, for comments and dead code that render nothing, would make the release's checks run on a different program from the one that passed; recorded instead. | agent-autonomous |
| **The release-time outsider test found six provenance records naming the batch-2 lockfile digest with no appended section, and a digest check that never looked there.** Each record has its section now (the lockfile changed at `fa71b3b`, batch 5, when scikit-learn was added; the results were reproduced byte for byte under the current lockfile by both clean-room runs), and `check_hashes` verifies the `environment:` block's lockfile digest as well as the `script:` blocks' — a methodological change, committed as such. | `provenance-record.md` says the digest in a record has to be the digest of the file that is there, and `validate.md` said the check enforced it; it enforced it for scripts only, so the statement was true as prose and false as behaviour — the class batch 16 (`freeze`), batch 19 (`cleanroom.sh`) and batch 20 (`settings.json`) each found one of. Fixed at the cause and at the check, not by weakening either. | agent-autonomous |
| **Recorded and not fixed: a new alternatives child added today has no mechanised route to a new development-manifest version while the main path is unchanged.** `02_plan_manifest.py` re-plans only when the frozen main path differs from the tree's, so a contributor who follows the tooling's messages ends at a hand edit of a frozen file or a change to the planner. | Found by the outsider test (its question 4). Batch 19 fixed the messages so the dead end is named; mechanising the route is a change to the planner that a release batch should not make. Left for the human, with the observation that after the held-out year is opened a new path cannot enter the phase-E set at all, so the practical cost is confined to development-set work. | agent-autonomous (recording); open (whether to build the route) |
| **The clean-room reproduction was run again at release rather than carried over from batch 19, and passed without a fix**: cloned from `0642af0`, 0 inputs differing on checkout, 289 results byte-identical, 10 declared varying, 0 differing, 0 missing, `analysis/run.sh` in 4,757 s. One defect in the check's own record noted, not fixed: `cleanroom.sh` writes HEAD at summary time into `summary.json`, so a run that overlaps later commits records the wrong commit (`e5dd305` there; the console log has the clone's `0642af0`). | Two `analysis/` scripts and every `run.sh` changed after the batch-19 run; `/release` says the check precedes anything counting as released. The script is left as it is because the artefact that passed was produced by the committed script; capturing the clone commit at clone time is a one-line change for the next batch that touches it. | agent-autonomous |
| **The agent does not push.** Row 20 closes on everything but the push, which waits for the human with the scan's findings in front of them; `/release` says "never pushes without asking" and this batch ran without the human present. The citable, versioned snapshot with a persistent identifier that `/release` asks for needs an archive account (Zenodo or equivalent) and is likewise the human's step; a local annotated tag marks the release commit so the snapshot has something to point at. | `/commit-run` and `/release` both reserve the push for the human; the plan's row 20 says "push", but the plan was written before the scan's findings existed, and a push is not reversible. | human (the push and the deposit); agent-autonomous (the tag) |

### 2026-09-23 — batch 20: the push

| Decision | Basis | Agency |
|---|---|---|
| **The release is pushed**: `main` moved from `c8fd3f5` (batch 10) to `b5fb880` on the public remote, with the tag `v1.0-release`, 42 commits. | The human's instruction, given after the batch's report had put the scan's two findings in front of them — the credential-shaped filename in the history holding no credential, and the home-directory path in four tracked files — and after confirming that every ledger row was done. Neither finding was changed before the push. | human-set |
...
| 20 | F — Release | Write the manuscript section(s) this project supports; run the release scan; push. | done — the manuscript, the reproducibility report, the release scan, the clean-room and outsider checks re-run at release, the hierarchical report rebuilt, the release commit tagged `v1.0-release`; **pushed at the human's instruction on 2026-09-23** after the scan's findings were reported (§4b). The citable deposit (GitHub release + Zenodo DOI) remains the human's. |
...
### Batch 20 — the release: the manuscript, the reproducibility report, the scan, both checks re-run

- [[26-09-23_b20_release]]
- [[26-09-23_twoStageDengueLaosManuscript]]
- [[26-09-23_reproducibilityReport]]
