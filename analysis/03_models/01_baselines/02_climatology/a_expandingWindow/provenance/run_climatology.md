...
**What it establishes.** Run under `persistence_negBinomialFloor`, where the driver takes this
fork's main-path child, it scores **24.337** — identical to the main path and to the five setup
rows. Its own sibling's row, `climatology_frozenWindow`, is where it is absent, and the
difference between the two is 0.532 CRPS, inside the 0.565 floor.
...
script:              scripts/run_climatology.py   (unchanged)
                     analysis/03_models/scripts/lib/chap_eval.py
                     sha256:b05916bff2567b79571ba2ce27dbf26ce5cf5e6540c278ba5b570df1907688f5
invocation:          unchanged: "$PYTHON" scripts/run_climatology.py, from the node
...
of the library. Batch 18's clean-room run rebuilt the main path from a fresh clone at
`ad7e64f` with this library and climatology came back at **24.336908636118597** against an
archived 24.336908636118597 (`AI-generated/validation/26-09-01_cleanroom.md`). Every run of
this node from batch 13 onward used this version directly.
