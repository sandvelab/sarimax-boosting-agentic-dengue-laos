...
datasets." Also reads the three prior candidates' own `comparison.json` (their closed
records, not recomputed) to report all four stage-2 candidates side by side, and separately
flags the comparison this node exists to make -- against `a_linearLags` specifically, since
this is that candidate's family with climate added, isolating the input-richness question.
"""
...
def main() -> None:
    ...
    delta_vs_a = two_stage_crps - a["two_stage_ensemble"]["mean_crps"]
    pct_vs_a = 100 * delta_vs_a / a["two_stage_ensemble"]["mean_crps"]
    ...
    summary = {
        ...
        "vs_a_linearLags": {
            "a_linearLags_mean_crps": a["two_stage_ensemble"]["mean_crps"],
            "delta_crps": delta_vs_a,
            "pct_change_vs_a_linearLags": pct_vs_a,
            "climate_helps_over_minimal_input": bool(delta_vs_a < 0),
        },
    }
    (RESULTS / "comparison.json").write_text(json.dumps(summary, indent=2) + "\n")

    # Side-by-side of all four stage-2 candidates built so far, from each one's own closed
