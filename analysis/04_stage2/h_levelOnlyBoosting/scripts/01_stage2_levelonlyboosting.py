#!/usr/bin/env python3
"""Stage 2 candidate `h_levelOnlyBoosting` (batch 14): the minimal input.

**Why.** `05_residualStructure`'s cross-validated predictability test found that only the
forecast-level features beat stage 1 alone on the test cells, and the stability run
(`06_stability`, row `stage2=g_level_month_horizon_only`) found the same restriction moved
the development margin from -3.41% to -6.53%. The recent-residual, cross-province,
incidence and reporting-level terms may be adding variance the pooled booster does not
need. Stage 2 here sees horizon indicators, target-month indicators, stage 1's forecast
level relative to the province's residual scale, and that level times the horizon --
nothing about the residual history at the origin.
...
argument for each; this one changes only the feature set (`features="level_only"`).
...
NODE = Path(__file__).resolve().parents[1]  # analysis/04_stage2/h_levelOnlyBoosting
...
STAGE2 = Stage2Config(features="level_only")
...
def main() -> None:
    ...
    summary = {
        "model": "stage1 SARIMAX(1,1,1)x(1,0,0,12) + gradient-boosted trees on the standardised h-step in-window out-of-sample error, features: horizon, target month, forecast level relative to residual scale (level-only); correction zhat*se, clipped at zero; sigma unchanged",
