#!/usr/bin/env python3
"""Stage 2 candidate `i_boundedBoosting` (batch 14): the bounded correction.

**Why.** Batch 10 traced both new candidates' Savannakhet loss to a mechanism: stage 1's
standard error there (200-414) is tens of times the 2008-09 case level, so a standardised
correction of +0.4-0.6 becomes 130-190 cases added to a forecast of 50 for an actual near
zero. The correction inherits stage 1's spread miscalibration. This candidate bounds it
relative to the forecast level: |correction| <= max(stage-1 mean, 10). The stability run
(`06_stability`, row `stage2=g_bounded_correction`) found this moved the development margin
from -3.41% to -5.85%; a floor of 10 cases keeps a correction possible where the forecast
is near zero. The floor is a judgment call, logged, and a perturbation.
...
argument for each; this one changes only the correction bound (`bounded_correction=True`, `bound_floor=10`).
...
NODE = Path(__file__).resolve().parents[1]  # analysis/04_stage2/i_boundedBoosting
...
STAGE2 = Stage2Config(bounded_correction=True, bound_floor=10.0)
...
def main() -> None:
    ...
    summary = {
        "model": "stage1 SARIMAX(1,1,1)x(1,0,0,12) + gradient-boosted trees on the standardised h-step in-window out-of-sample error, full forecast-time features; correction zhat*se bounded to |correction| <= max(stage-1 mean, 10), clipped at zero; sigma unchanged",
