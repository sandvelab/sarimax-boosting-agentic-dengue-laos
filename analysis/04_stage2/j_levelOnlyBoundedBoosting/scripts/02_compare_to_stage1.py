...
against `h_levelOnlyBoosting` on the same cells, and all ten candidates side by side with the pre-registered main-path rule applied.
"""
...
PARENT_COMPARISON = STAGE2 / "h_levelOnlyBoosting" / "results" / "comparison.json"
...
def main() -> None:
    ...
    summary = {
        ...
        "vs_h_levelOnlyBoosting": {"parent_mean_crps": parent_crps, "delta_crps": cand["mean_crps"] - parent_crps,
    ...
    # All ten stage-2 candidates side by side, from each sibling's own comparison file.
    side = {"stage1_alone_mean_crps": stage1_crps, "stage1_alone_coverage_empirical": variants["stage1"]["coverage_empirical"]}
    for name in ("a_linearLags", "b_gradientBoosting", "c_bayesianRidge", "d_linearClimate", "e_pooledRandomForest",
                 "f_oosErrorRidge", "g_oosErrorBoosting", "h_levelOnlyBoosting", "i_boundedBoosting"):
        c = json.loads((STAGE2 / name / "results" / "comparison.json").read_text())
        side[name] = {"mean_crps": c["two_stage_ensemble"]["mean_crps"], "pct_change_vs_stage1": c["pct_change_vs_stage1"],
                      "coverage_empirical": c["two_stage_ensemble"]["coverage_empirical"],
                      "n_splits_improved_of_8": c.get("n_splits_improved_of_8")}
    side["j_levelOnlyBoundedBoosting"] = {"mean_crps": cand["mean_crps"], "pct_change_vs_stage1": cand["pct_change_vs_stage1"],
                                          "coverage_empirical": cand["coverage_empirical"], "n_splits_improved_of_8": n_splits_improved}
    names = [k for k in side if not k.startswith("stage1_alone")]
    side["ranking_by_mean_crps"] = sorted(names, key=lambda k: side[k]["mean_crps"])
    side["candidates_clearing_both_bars (beat stage 1 on CRPS, coverage not worse)"] = [
        k for k in side["ranking_by_mean_crps"] if side[k]["mean_crps"] < stage1_crps
        and side[k]["coverage_empirical"] >= side["stage1_alone_coverage_empirical"]]
    # The pre-registered main-path rule (plan section 4b, 2026-09-21), applied to g, h, i, j: lowest development
    # mean CRPS with coverage not worse than stage 1's; a tie within 0.1 CRPS broken by more splits improved,
    # then by the simpler configuration (fewer changes from g).
    simplicity = {"g_oosErrorBoosting": 0, "h_levelOnlyBoosting": 1, "i_boundedBoosting": 1, "j_levelOnlyBoundedBoosting": 2}
    eligible = [k for k in simplicity if side[k]["mean_crps"] < stage1_crps
                and side[k]["coverage_empirical"] >= side["stage1_alone_coverage_empirical"]]
    best = min(side[k]["mean_crps"] for k in eligible)
    tied = [k for k in eligible if side[k]["mean_crps"] <= best + 0.1]
    tied.sort(key=lambda k: (-(side[k]["n_splits_improved_of_8"] or 0), simplicity[k]))
    side["main_path_rule"] = ("among g, h, i, j: lowest development mean CRPS with 90% coverage >= stage 1's; tie within "
                              "0.1 CRPS -> more splits improved -> fewer changes from g")
    side["main_path_by_rule"] = tied[0]
    side["tied_within_0.1"] = tied
    (RESULTS / "all_candidates_comparison.json").write_text(json.dumps(side, indent=2) + "\n")
    print(json.dumps({k: side[k] for k in ("ranking_by_mean_crps", "main_path_by_rule", "tied_within_0.1")}, indent=2))
