...
The step list per row substitutes the moved child for the main one. **Since batch 20 that
construction and its execution live in `lib/driver.py`**, because `06_external` — the
reported model, unchanged, on the sibling countries — is the same operation on a different
dataset, and an external check that ran a second implementation of the pipeline would not
be a check. The table below is what that library encodes; this file decides which manifest
is being run, what the seal on the held-out year is, and what a row that cannot run records.
...
# Building a row's step list and executing it moved to `lib/driver.py` in batch 20, when
# `06_external` became a second caller of the same operation. What stayed here is the
# perturbation set's own reasoning: which manifest, what the seal is, and what a row that
# cannot run records.
...
def main() -> None:
    ...
    for row in rows:
        ...
        if row["built"] != "True":
            ...
            # writes it works to.
            if args.dry_run:
                run_row(row, forks, dry=True, logs=LOGS)
            outcomes.append({"combination": row["combination"],
                             "status": "not-built: the child has no scripts yet",
                             "seconds": "", "steps": "", "log": "", "at": ""})
            continue
        outcomes.append(run_row(row, forks, args.dry_run, logs=LOGS,
                                full_main=holdout))
