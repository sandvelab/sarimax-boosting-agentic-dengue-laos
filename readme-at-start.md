...
| Storage budget | **Not a constraint.** A few gigabytes for the whole repository is fine (human-set, 2026-08-29, on batch 22's question). Nothing is pruned and no batch plans around disk; `/annotate-criticality` keeps annotating so that a future decision would be targeted. For scale: every combination on disk is 660.2 MB, against a projected 1 169.4 MB for all of tier 1 across both datasets (`analysis/05_stability/results/manifest_notes.json`). |
