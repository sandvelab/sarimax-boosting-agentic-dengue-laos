...
1. `analysis/run.sh` reproduces the reported analysis from a clean environment — and, since
   ...
   held-out year. **As of batch 25 this is verified for the reported main path and the whole
   development set, and `analysis/run.sh` does not currently run to the end from a clean
   checkout at all.** Batch 25's clean-room run completed all 32 development rows and then
   **exited 1** at the freeze check: the tier-2 rows of the development manifest are not
   planned from the tree but *selected* from tier 1's skill scores, which divide by the
   unseeded reference, and a second draw re-selected six of the eight pairs — so the frozen
   phase-E set has a derivation a clean checkout cannot reproduce, and batch 24's freeze
   defence correctly refused to rewrite it. **The phase-E half has still never been executed
   from a clean checkout**, for a stated reason rather than an interruption. What *is*
